...
const ProductCard=({product}:Iprops )=>{
    ...
    return (
        ...
        <div className="max-w-sm md:max-w-lg mx-auto md:mx-0 border rounded-md flex flex-col p-2 ">
            <Image className="rounded-md lg:object-cover h-52 w-full" imageUrl={imageURL} alt="Product Name"/>
<h2>{title}</h2>
<p>{sliceText(description)}</p>
        ...
        <div className="flex items-center justify-between mb-2 ">
            ...
            <Image className="rounded-full object-bottom h-10 w-10" imageUrl={category.imageURL} alt="Product Name"/>
        </div>

        <div className="flex items-center justify-between space-x-2">
            <Button className="bg-indigo-700"  width ={"w-full"} onClick={()=>{
                console.log("edit");
